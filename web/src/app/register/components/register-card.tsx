...
export function RegisterCard() {
  ...
  if (isLoading) {
    return (
      <div className="yan-panel flex items-center justify-center rounded-2xl p-10">
            ...
            <div className="min-h-0 flex-1 overflow-y-auto rounded-2xl border border-[rgba(37,99,235,0.14)] bg-[#1e3a8a] p-4 font-mono text-xs leading-6 shadow-[inset_0_1px_0_rgba(255,255,255,0.05)]">
              {logs.length === 0 ? (
                ...
                logs.slice().reverse().map((item, index) => (
                  <div key={`${item.time}-${index}`} className={item.level === "red" ? "text-red-300" : item.level === "green" ? "text-emerald-300" : item.level === "yellow" ? "text-amber-300" : "text-stone-200"}>
